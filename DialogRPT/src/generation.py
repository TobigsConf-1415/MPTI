...
class GPT2Generator:
            ...
            for i in range(tokens.shape[0]):
                for j in range(topk):
                    ...
                    if next_token[i, j] == self.ix_EOS:
                        ...
                        hyp = self.tokenizer.decode(seq[:-1], skip_special_tokens=False)   # don't include EOS
    ...
    def predict_sampling(self, cxt, temperature=1, n_hyp=5, max_t=30):
        ...
        for i in range(n_hyp):
            ...
            hyp = self.tokenizer.decode(seqs[i][:-1], skip_special_tokens=False)   # strip EOS
            ret.append((prob, hyp))
        return ret
...
if __name__ == "__main__":
    ...
    if args.path_ranker is None:
        ...
        from src.score import get_model
